...
# Registriere Forward-Hooks auf Transformer-Decoder-Layern
...
# Nun extrahieren wir die Pixel-Embeddings mit eindeutiger ID
...
def extract_pixel_embedding_map(model: torch.nn.Module, batched_input: Dict[str, Any], 
    ...
    # Hooks für Encoder/Backbone wichtig
    for name, module in model.named_modules():
...
# DIe Query-Embeddings werden in organisierter Form gespeichert, so dass wir auf die Ergebbisse später gut zugreiden können.
...
# Vorbereiten der Bilder auf MAskDINO-Modell-Eingabe
...
# Führt die vorher erstellen Schritte aus und sorgt für einen reibungslosen Ablauf.
