...
def _maybe_capture_parent_projections(parent_block: nn.Module, attn_cache: object) -> None:
    try:
        ...
        if isinstance(vp, nn.Linear) and isinstance(op, nn.Linear) and isinstance(n_heads, int) and isinstance(d_model, int) and d_model % n_heads == 0:
            Dh = d_model // n_heads
            E = d_model
            W_v = vp.weight.detach()
            W_o = op.weight.detach()
            setattr(attn_cache, 'W_V_deform', W_v.view(E, n_heads, Dh).permute(1, 2, 0).contiguous())
            setattr(attn_cache, 'W_O_deform', W_o.t().contiguous().view(E, n_heads, Dh).permute(1, 2, 0).contiguous())
    except Exception:
        pass

def attach_msdeformattn_capture(root: nn.Module, attn_cache: Optional[object]):
    handles: List[torch.utils.hooks.RemovableHandle] = []
    MSDeformAttn = _resolve_msdeformattn_class()
    if MSDeformAttn is None:
        # Deutliche Diagnose, warum kein Deformable-Attention-Capture stattfinden kann
        try:
    ...
    def _wrap_forward(mod: nn.Module):
        """Wrappe MSDeformAttn.forward mit der tatsächlichen Signatur von MaskDINO.
        ...
        def _patched_forward(query: Tensor, reference_points: Tensor, input_flatten: Tensor, input_spatial_shapes: Tensor, input_level_start_index: Tensor, input_padding_mask: Optional[Tensor] = None, *args, **kwargs):
            out = orig_forward(query, reference_points, input_flatten, input_spatial_shapes, input_level_start_index, input_padding_mask, *args, **kwargs)
            try:
                if attn_cache is not None:
                    # Rekonstruiere die internen Größen gemäß Implementierung
                    N, Len_q, _ = query.shape
                    N2, S, _ = input_flatten.shape
                    if N2 != N:
                        # Unplausibel, dennoch abbrechen
                    ...
                    # Sampling-Offsets, -Gewichte und -Lokationen
                    sampling_offsets = mod.sampling_offsets(query).view(N, Len_q, Hh, mod.n_levels, mod.n_points, 2)
                    attention_weights = mod.attention_weights(query).view(N, Len_q, Hh, mod.n_levels * mod.n_points)
                    attention_weights = F.softmax(attention_weights, -1).view(N, Len_q, Hh, mod.n_levels, mod.n_points)
                    if reference_points.shape[-1] == 2:
                        offset_normalizer = torch.stack([input_spatial_shapes[..., 1], input_spatial_shapes[..., 0]], -1)
                        sampling_locations = reference_points[:, :, None, :, None, :] + sampling_offsets / offset_normalizer[None, None, None, :, None, :]
                    elif reference_points.shape[-1] == 4:
                        sampling_locations = reference_points[:, :, None, :, None, :2] + sampling_offsets / mod.n_points * reference_points[:, :, None, :, None, 2:] * 0.5
                    else:
    ...
    for m in root.modules():
        if isinstance(m, MSDeformAttn):
            ...
            found += 1
    if found == 0:
        # Keine Instanzen im gewählten Layer/Teilbaum gefunden – Hinweis ausgeben
        try:
